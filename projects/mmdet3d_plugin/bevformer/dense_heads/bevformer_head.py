...
from mmdet3d.registry import MODELS, TASK_UTILS as MMDET3D_TASK_UTILS
...
class BEVFormerHead(DETRHead):
    ...
    def __init__(self,
        ...
        try:
            self.bbox_coder = MMDET3D_TASK_UTILS.build(bbox_coder)
        except Exception:
        ...
        detr_head_sig = inspect.signature(DETRHead.__init__)
        kwargs = kwargs.copy()
        if 'num_query' in kwargs and 'num_query' not in detr_head_sig.parameters \
                and 'num_queries' in detr_head_sig.parameters:
            kwargs['num_queries'] = kwargs.pop('num_query')
        if 'num_queries' in kwargs and 'num_queries' not in detr_head_sig.parameters \
                and 'num_query' in detr_head_sig.parameters:
            kwargs['num_query'] = kwargs.pop('num_queries')
        if 'num_query' in kwargs:
            self.num_query = kwargs['num_query']
        elif 'num_queries' in kwargs:
            self.num_query = kwargs['num_queries']
        pos_enc_cfg = copy.deepcopy(kwargs.get('positional_encoding', None))
        accepted = set(detr_head_sig.parameters.keys())
        accepted.discard('self')
        kwargs = {k: v for k, v in kwargs.items() if k in accepted}
        if 'transformer' in detr_head_sig.parameters:
            try:
                super(BEVFormerHead, self).__init__(
                    *args, transformer=transformer, **kwargs)
            except TypeError as e:
                if 'num_query' in str(e) and 'num_query' in kwargs:
                    kwargs = kwargs.copy()
                    kwargs['num_queries'] = kwargs.pop('num_query')
                    super(BEVFormerHead, self).__init__(
                        *args, transformer=transformer, **kwargs)
            ...
            # mmdet 3.x path: DETRHead no longer accepts `transformer` kwarg.
            # Build transformer first so BEVFormerHead._init_layers can access
            # decoder depth during super().__init__.
            try:
                built_transformer = MODELS.build(transformer)
            except Exception:
                built_transformer = MMDET_MODELS.build(transformer)
            # Bypass nn.Module registration check before Module.__init__.
            self.__dict__['transformer'] = built_transformer
            try:
                super(BEVFormerHead, self).__init__(*args, **kwargs)
            except TypeError as e:
                if 'num_query' in str(e) and 'num_query' in kwargs:
                    kwargs = kwargs.copy()
                    kwargs['num_queries'] = kwargs.pop('num_query')
                    ...
                    raise
            # Register transformer as a submodule after parent init.
            self.transformer = built_transformer

        direct_cfg = copy.deepcopy(pos_enc_cfg)
        direct_cfg.pop('type', None)
        self.positional_encoding = LearnedPositionalEncoding(**direct_cfg)

        self.code_weights = nn.Parameter(torch.tensor(
                 self.code_weights, requires_grad=False), requires_grad=False)

        if not hasattr(self, 'sampler'):
            self.sampler = MMDET_TASK_UTILS.build(dict(type='PseudoSampler'))
    ...
    def _get_target_single(self, cls_score, bbox_pred, gt_labels, gt_bboxes, gt_bboxes_ignore=None):
        ...
        gt_instances.labels_3d = gt_labels
